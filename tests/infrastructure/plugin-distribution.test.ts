...
describe('Plugin Distribution - Skills', () => {
  const skillPath = path.join(projectRoot, 'plugin/skills/smart-explore/SKILL.md');

  it('should include plugin/skills/smart-explore/SKILL.md', () => {
  ...
  it('should reference smart_search in the skill workflow', () => {
    const content = readFileSync(skillPath, 'utf-8');
    expect(content).toContain('smart_search');
    expect(content).toContain('smart_outline');
    expect(content).toContain('smart_unfold');
  });
});
...
describe('Plugin Distribution - hooks.json Integrity', () => {
  ...
  it('should include CLAUDE_PLUGIN_ROOT fallback in all hook commands (#1215)', () => {
    const hooksPath = path.join(projectRoot, 'plugin/hooks/hooks.json');
    const parsed = JSON.parse(readFileSync(hooksPath, 'utf-8'));
    const expectedFallbackPath = '$HOME/.claude/plugins/marketplaces/vibelearn/plugin';

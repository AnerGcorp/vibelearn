...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds vibelearn server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['vibelearn']).toBeDefined();
      expect(config.mcpServers['vibelearn'].command).toBe('node');
      expect(config.mcpServers['vibelearn'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding vibelearn', () => {
    ...
    it('updates existing vibelearn server path', () => {
      // First config
      ...
      expect(config.mcpServers['vibelearn'].args).toEqual([newPath]);
    });
  ...
  describe('removeMcpConfig', () => {
    it('removes vibelearn server from config', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);
      removeMcpConfig(mcpJsonPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['vibelearn']).toBeUndefined();
    });

    it('preserves other servers when removing vibelearn', () => {
      // Setup: both servers
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] },
          'vibelearn': { command: 'node', args: ['/mcp.cjs'] }
        }
      };
      ...
      expect(updated.mcpServers['vibelearn']).toBeUndefined();
    });
    ...
    it('does nothing if vibelearn not in config', () => {
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] }
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['vibelearn'].args).toEqual([pathWithSpaces]);
    });

    it('handles Windows-style path', () => {
      const windowsPath = 'C:\\Users\\atajanoviic\\.claude\\plugins\\mcp-server.cjs';
      configureCursorMcp(mcpJsonPath, windowsPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['vibelearn'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['vibelearn'].args).toEqual([specialPath]);

      // Verify it survives JSON round-trip
      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['vibelearn'].args![0]).toBe(specialPath);
    });
  });
});

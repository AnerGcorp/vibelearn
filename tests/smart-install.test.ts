...
const TEST_DIR = join(tmpdir(), `vibelearn-smart-install-test-${process.pid}`);
...
describe('smart-install resolveRoot logic', () => {
  ...
  it('should prefer CLAUDE_PLUGIN_ROOT when it contains package.json', () => {
    const cacheDir = createDir('cache/anergcorp/vibelearn/10.0.0');
  ...
  it('should detect cache-based install paths', () => {
    // Cache installs have paths like ~/.claude/plugins/cache/anergcorp/vibelearn/<version>/
    const cacheDir = createDir('plugins/cache/anergcorp/vibelearn/10.3.0');
    createPackageJson(cacheDir);
...
describe('smart-install stdout JSON output (#1253)', () => {
  ...
  it('should produce valid JSON when run with plugin disabled', () => {
    ...
    const settingsDir = join(tmpdir(), `vibelearn-test-settings-${process.pid}`);
    const settingsFile = join(settingsDir, 'settings.json');
    mkdirSync(settingsDir, { recursive: true });
    writeFileSync(settingsFile, JSON.stringify({
      enabledPlugins: { 'vibelearn@anergcorp': false }
    }));

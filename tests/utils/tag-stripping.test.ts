 ...
 *
 * Tests the tag privacy system for <private>, <vibelearn-context>, and <system_instruction> tags.
...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <vibelearn-context> tag', () => {
        const input = 'public content <vibelearn-context>injected context</vibelearn-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <vibelearn-context>context</vibelearn-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <vibelearn-context> blocks', () => {
        const input = '<vibelearn-context>ctx1</vibelearn-context><vibelearn-context>ctx2</vibelearn-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <vibelearn-context>c${i}</vibelearn-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        // Tags are stripped but spaces between them remain
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<vibelearn-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<vibelearn-context>all is context</vibelearn-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <vibelearn-context>more</vibelearn-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <vibelearn-context> tags', () => {
        const input = `start
<vibelearn-context>
# Recent Activity
- Item 1
...
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip vibelearn-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<vibelearn-context>injected</vibelearn-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <vibelearn-context>context data</vibelearn-context>',
          status: 'success'
        };

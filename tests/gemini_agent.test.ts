...
describe('GeminiAgent', () => {
  ...
  beforeEach(() => {
    ...
    loadFromFileSpy = spyOn(SettingsDefaultsManager, 'loadFromFile').mockImplementation(() => ({
      ...
      VIBELEARN_GEMINI_API_KEY: 'test-api-key',
      VIBELEARN_GEMINI_MODEL: 'gemini-2.5-flash-lite',
      VIBELEARN_GEMINI_RATE_LIMITING_ENABLED: rateLimitingEnabled,
      VIBELEARN_DATA_DIR: '/tmp/vibelearn-test',
    }));

    getSpy = spyOn(SettingsDefaultsManager, 'get').mockImplementation((key: string) => {
      if (key === 'VIBELEARN_GEMINI_API_KEY') return 'test-api-key';
      if (key === 'VIBELEARN_GEMINI_MODEL') return 'gemini-2.5-flash-lite';
      if (key === 'VIBELEARN_GEMINI_RATE_LIMITING_ENABLED') return rateLimitingEnabled;
      if (key === 'VIBELEARN_DATA_DIR') return '/tmp/vibelearn-test';
    ...
    const mockSessionStore = {
      ...
      getSessionById: mock(() => ({ memory_session_id: 'vl-session-123' })), // Required by ResponseProcessor.ts for FK fix
  ...
  it('should initialize with correct config', async () => {
    const session = {
      ...
      memorySessionId: 'vl-session-123',
  ...
  it('should respect rate limits when rate limiting enabled', async () => {
    ...
    // Note: VIBELEARN_GEMINI_RATE_LIMITING_ENABLED !== 'false' means enabled

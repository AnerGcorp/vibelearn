...
const TEST_DATA_DIR = path.join(homedir(), '.vibelearn-test');
...
describe('Windows-specific behavior', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.VIBELEARN_MANAGED;
  });
  ...
  it('should detect Windows managed worker mode correctly', () => {
    ...
    process.env.VIBELEARN_MANAGED = 'true';

    const isWindows = process.platform === 'win32';
    const isManaged = process.env.VIBELEARN_MANAGED === 'true';

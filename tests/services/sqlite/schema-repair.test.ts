 ...
 * machines running different vibelearn versions
 */
...
function tempDbPath(): string {
  return join(tmpdir(), `vibelearn-test-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
}
...
describe('Schema repair on malformed database', () => {
  it('should repair a database with an orphaned index referencing a non-existent column', () => {
    ...
    try {
      ...
      // Step 4: Open via VibeLearnDatabase — it should auto-repair
      const repaired = new VibeLearnDatabase(dbPath);
  ...
  it('should handle a fresh database without triggering repair', () => {
    const dbPath = tempDbPath();
    try {
      const db = new VibeLearnDatabase(dbPath);
...
`);
      ...
      // VibeLearnDatabase must repair and fully initialize despite missing schema_versions
      const repaired = new VibeLearnDatabase(dbPath);
      const tables = repaired.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name")
        .all() as { name: string }[];
      const tableNames = tables.map(t => t.name);
  ...
  it('should preserve existing data through repair and re-migration', () => {
    ...
    try {
      ...
      // Step 3: Repair via VibeLearnDatabase
      const repaired = new VibeLearnDatabase(dbPath);

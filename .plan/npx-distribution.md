# Plan: NPX Distribution + Universal IDE/CLI Coverage for vibelearn
...
2. **IDE coverage is limited**: vibelearn only supports Claude Code (plugin) and Cursor (hooks installer). The AI coding tools landscape has exploded — Gemini CLI (95k stars), OpenCode (110k stars), Windsurf (~1M users), Codex CLI, Antigravity, Goose, Crush, Copilot CLI, and more all support extensibility.
...
- **All IDEs share a single worker on port 37778**: One worker serves all integrations. Session source (which IDE) is tracked via the `source` field in hook payloads. No per-IDE worker instances.
- **This npx CLI fully replaces the old `vibelearn-installer`**: Not a supplement — the complete replacement.

## Solution

`npx vibelearn` becomes a unified CLI: install, configure any IDE, manage the worker, search memory.

```
npx vibelearn                          # Interactive install + IDE selection
npx vibelearn install                  # Same as above
npx vibelearn install --ide windsurf   # Direct IDE setup
npx vibelearn start / stop / status    # Worker management
npx vibelearn search <query>           # Search memory from terminal
npx vibelearn transcript watch         # Start transcript watcher
```
...
2. **Create `src/npx-cli/index.ts`** — a Node.js CLI router (NOT Bun) with command categories:
   ...
   **Install commands** (pure Node.js, no Bun required):
   - `npx vibelearn` or `npx vibelearn install` → interactive install (IDE multi-select)
   - `npx vibelearn install --ide <name>` → direct IDE setup (only for implemented IDEs; unimplemented ones error with "Support for <name> coming soon")
   - `npx vibelearn update` → update to latest version
   - `npx vibelearn uninstall` → remove plugin and IDE configs
   - `npx vibelearn version` → print version

   **Runtime commands** (delegate to Bun via installed plugin):
   - `npx vibelearn start` → spawns `bun worker-service.cjs start`
   - `npx vibelearn stop` → spawns `bun worker-service.cjs stop`
   - `npx vibelearn restart` → spawns `bun worker-service.cjs restart`
   - `npx vibelearn status` → spawns `bun worker-service.cjs status`
   - `npx vibelearn search <query>` → hits `GET http://localhost:37778/api/search?q=<query>`
   - `npx vibelearn transcript watch` → starts transcript watcher

   **Runtime commands must check for installation first**: If plugin directory doesn't exist at `~/.claude/plugins/marketplaces/anergcorp/`, print "vibelearn is not installed. Run: npx vibelearn install" and exit.

3. **The install flow** (fully replaces git clone + build):
   - Detect the npm package's own location (`import.meta.url` or `__dirname`)
   - Copy `plugin/` from the npm package to `~/.claude/plugins/marketplaces/anergcorp/`
   - Copy `plugin/` to `~/.claude/plugins/cache/anergcorp/vibelearn/<version>/`
...
- `npx vibelearn install` copies plugin to correct directories on macOS, Linux, and Windows
- Auto-detection finds installed IDEs
- `npx vibelearn start/stop/status` work after install
- `npx vibelearn search "test"` returns results
- `npx vibelearn start` before install prints helpful error message
- `npx vibelearn update` and `npx vibelearn uninstall` work correctly
- `npx vibelearn version` prints version
...
2. **Create Gemini CLI hooks installer** at `src/services/integrations/GeminiCliHooksInstaller.ts`:
   ...
   - Hook config format (verified against official docs):
     ```json
     {
       "hooks": {
         "AfterTool": [{
           "matcher": "*",
           "hooks": [{ "name": "vibelearn", "type": "command", "command": "<path-to-hook-script>", "timeout": 5000 }]
         }]
       }
     }
     ```
   ...
   - Context injection via `~/.gemini/GEMINI.md` (append vibelearn section with `<vibelearn-context>` tags, same pattern as CLAUDE.md)
...
- `npx vibelearn install --ide gemini-cli` merges hooks into `~/.gemini/settings.json`
...
- `GEMINI.md` gets vibelearn context section
...
export const VibeLearnPlugin: Plugin = async (ctx) => {
...
return {
  tool: {
    vibelearn_quiz: tool({
      description: "Search vibelearn memory database",
      args: { query: tool.schema.string() },
      async execute(args, context) {
        // context: { sessionID, messageID, agent, directory, worktree, abort, metadata, ask }
        const response = await fetch(`http://localhost:37778/api/search?q=${encodeURIComponent(args.query)}`)
...
1. **Create OpenCode plugin** at `src/integrations/opencode-plugin/index.ts`:
   ...
   | Mechanism | Event | Map to vibelearn |
   |-----------|-------|-------------------|
   ...
   - Register `vibelearn_quiz` custom tool using correct `tool({ description, args, execute })` API
   - Hit `localhost:37778` API endpoints from the plugin
...
3. **Create OpenCode setup in installer** (two options, prefer file-based):
   - **Option A (file-based):** Copy plugin to `~/.config/opencode/plugins/vibelearn.ts` (auto-loaded at startup)
   - **Option B (npm-based):** Add to `~/.config/opencode/opencode.json` under `"plugin"` array: `["vibelearn"]`
   - Config also supports JSONC (`opencode.jsonc`) and legacy `config.json`
   - Context injection: Append to `~/.config/opencode/AGENTS.md` (or create it) with `<vibelearn-context>` tags
...
- `npx vibelearn install --ide opencode` registers the plugin (file or npm)
...
- `vibelearn_quiz` custom tool works in OpenCode sessions
...
| Event | Can Block? | Map to vibelearn | Use |
|-------|-----------|-------------------|-----|
...
2. **Create Windsurf hooks installer** at `src/services/integrations/WindsurfHooksInstaller.ts`:
   ...
   - Context injection via `.windsurf/rules/vibelearn-context.md` (workspace-level; Windsurf rules are workspace-scoped)
...
- `npx vibelearn install --ide windsurf` creates hooks config at `~/.codeium/windsurf/hooks.json`
- Windsurf sessions are captured by the worker via post-action hooks
- `trajectory_id` is used as session identifier
- Context is injected via `.windsurf/rules/vibelearn-context.md` (under 6K char limit)
...
2. **Create Codex setup in installer**:
   - Write transcript-watch config to `~/.vibelearn/transcript-watch.json`
...
- `npx vibelearn install --ide codex` creates transcript watch config
- Codex sessions appear in vibelearn database
...
1. **Wire OpenClaw into the npx installer**:
   ...
   - Register in `~/.openclaw/openclaw.json` under `plugins.vibelearn`
...
- `npx vibelearn install --ide openclaw` registers the plugin
...
2. **Antigravity MCP setup**:
   ...
   - Context injection: `~/.gemini/GEMINI.md` (shared with Gemini CLI) and/or `.agent/rules/vibelearn-context.md`
...
5. **Roo Code MCP setup**:
   ...
   - Context injection: `.roo/rules/vibelearn-context.md`
...
- Each IDE can search vibelearn via MCP tools
...
1. Remove `vibelearn-installer` npm package (unpublish or mark deprecated with message pointing to `npx vibelearn`)
2. Update `install/public/install.sh` → redirect to `npx vibelearn`
...
4. `npx vibelearn start/stop/status/search` all work
5. `npx vibelearn update` updates correctly
6. `npx vibelearn uninstall` cleans up all IDE configs
7. `npx vibelearn version` prints version
8. `npx vibelearn start` before install shows helpful error

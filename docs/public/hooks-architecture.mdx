...
**Observe the main Claude Code session from the outside, analyze in the background, quiz yourself after.**

VibeLearn never modifies your prompts or Claude's responses. It attaches to 5 lifecycle events, captures what you build, and runs a local analysis pipeline when the session ends.
...
### SessionStart
- Ensures the worker service is running (waits up to 15s with retries)
...
Each step is independent — a failure in one does not block the next.

### SessionEnd
- Final cleanup
- Exits with code 0 (never blocks Claude Code from closing)
...
| `2` | Blocking error — fed to Claude for processing (rare) |

...
SQLite3. All vl-specific tables use the `vl_` prefix. Core session tables retain the original `sdk_sessions` and `observations` names.
...
|--------|------|-------------|
| `content_session_id` | TEXT UNIQUE | Claude Code's session ID (from hook input) |
...
Quiz questions generated per concept.
...
Tech stack detected per session.
...
Human-readable narrative per session.
...
Local streak cache. **Server is authoritative** — this is never trusted for official streak data.
...
{ id: 9, sql: `ALTER TABLE vl_concepts ADD COLUMN ...` }
```

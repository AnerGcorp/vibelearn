...
VibeLearn implements 5 Claude Code lifecycle hooks. Each hook is a TypeScript file compiled to an ESM script and run as a subprocess by Claude Code.
...
3. Returns `{ "continue": true }` — never modifies the prompt

**Privacy note**: Stripped content is never sent to the worker or stored.
...
**Performance**: Non-blocking. HTTP call is fire-and-forget — PostToolUse never delays Claude.

## Hook 4: Stop (Analysis Trigger)

**When**: Agent loop ends (user stops or Claude finishes).

#!/usr/bin/env bun
"use strict";var B=Object.create;var O=Object.defineProperty;var G=Object.getOwnPropertyDescriptor;var K=Object.getOwnPropertyNames;var J=Object.getPrototypeOf,X=Object.prototype.hasOwnProperty;var Z=(e,n,t,r)=>{if(n&&typeof n=="object"||typeof n=="function")for(let s of K(n))!X.call(e,s)&&s!==t&&O(e,s,{get:()=>n[s],enumerable:!(r=G(n,s))||r.enumerable});return e};var ee=(e,n,t)=>(t=e!=null?B(J(e)):{},Z(n||!e||!e.__esModule?O(t,"default",{value:e,enumerable:!0}):t,e));var H=require("readline"),L=require("bun:sqlite"),T=require("path"),h=require("os"),d=require("fs");function N(e,n){let t=e+n;return t===0?0:e/t}function k(e){return e>=.85?"senior":e>=.5?"mid":"junior"}function x(e,n){let t=Math.floor(Date.now()/1e3),r=e.query("SELECT * FROM vl_developer_profile WHERE concept_name = ?").get(n.conceptName);if(!r){let u=n.isCorrect?1:0,_=n.isCorrect?0:1,y=N(u,_),m=n.isCorrect?1:0,S=k(y);return e.run(`INSERT INTO vl_developer_profile
       ...
       VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?)`,[n.conceptName,n.category,t,t,u,_,S,m,y]),{concept_name:n.conceptName,category:n.category,first_seen_at:t,last_seen_at:t,encounter_count:1,correct_answers:u,incorrect_answers:_,current_level:S,streak_count:m,mastery_score:y}}let s=r.correct_answers+(n.isCorrect?1:0),c=r.incorrect_answers+(n.isCorrect?0:1),l=N(s,c),i=n.isCorrect?r.streak_count+1:0,a=k(l);return e.run(`UPDATE vl_developer_profile SET
       last_seen_at       = ?,
    ...
    LIMIT ?
  `).all(...c)}var R=["junior","mid","senior"],ne=3;function te(e){let n=R.indexOf(e);return n===-1||n>=R.length-1?"senior":R[n+1]}function oe(e){let n=R.indexOf(e);return n<=0?"junior":R[n-1]}function U(e,n){let t=e.query("SELECT current_level FROM vl_developer_profile WHERE concept_name = ?").get(n.conceptName),r=!t,s=t?.current_level??"junior",c=x(e,n),l=c.current_level,i=!1,a=!1;if(!r)if(n.isCorrect&&c.streak_count>=ne){let _=te(s);_!==s?(l=_,i=!0,e.run("UPDATE vl_developer_profile SET current_level = ?, streak_count = 0 WHERE concept_name = ?",[l,n.conceptName]),c.streak_count=0):(l=s,e.run("UPDATE vl_developer_profile SET current_level = ? WHERE concept_name = ?",[l,n.conceptName]))}else if(n.isCorrect)l=s,e.run("UPDATE vl_developer_profile SET current_level = ? WHERE concept_name = ?",[l,n.conceptName]);else{let _=oe(s);_!==s?(l=_,a=!0,e.run("UPDATE vl_developer_profile SET current_level = ? WHERE concept_name = ?",[l,n.conceptName])):(l=s,e.run("UPDATE vl_developer_profile SET current_level = ? WHERE concept_name = ?",[l,n.conceptName]))}return{profile:{...c,current_level:l},levelChanged:!r&&l!==s,previousLevel:s,promoted:i,demoted:a}}function Q(e,n,t,r){if(!e.follow_up_mid||!e.follow_up_mid.trim()||e.difficulty!=="junior"||!n||t!=="junior")return!1;let s=`followup_${e.id}`;return!r.has(s)}function j(e){return{id:`followup_${e.id}`,concept_name:e.concept_name,question_type:"open_ended",difficulty:"mid",snippet:e.snippet??null,question:e.follow_up_mid??"",options_json:null,correct:null,explanation:"(Follow-up \u2014 no single correct answer. Reflect on your understanding.)",follow_up_mid:null,is_follow_up:!0}}var se="0.1.6",b=process.env.VIBELEARN_DATA_DIR?process.env.VIBELEARN_DATA_DIR.replace("~",(0,h.homedir)()):(0,T.join)((0,h.homedir)(),".vibelearn"),A=(0,T.join)(b,"vibelearn.db"),I=(0,T.join)(b,"config.json");function z(){try{if((0,d.existsSync)(I))return JSON.parse((0,d.readFileSync)(I,"utf-8"))}catch{}return{}}function re(e){(0,d.existsSync)(b)||(0,d.mkdirSync)(b,{recursive:!0}),(0,d.writeFileSync)(I,JSON.stringify(e,null,2),"utf-8")}function q(){return(0,d.existsSync)(A)?new L.Database(A,{readonly:!0}):(console.log("No VibeLearn database found. Start a coding session first!"),null)}async function ie(){let e=q();if(!e)return;let n=e.query("SELECT COUNT(*) as count FROM vibelearn_session_summaries").get()?.count??0,t=e.query("SELECT COUNT(*) as count FROM vl_concepts").get()?.count??0,r=e.query("SELECT COUNT(*) as count FROM vl_questions").get()?.count??0,s=e.query(`
...
`),console.log("  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"),u++;try{let{randomUUID:v}=await import("crypto");if(c.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `,[v(),o.id,g,f?1:0,Y,Math.floor(Date.now()/1e3)]),!o.is_follow_up){let E=M({isCorrect:f,currentEaseFactor:o.ease_factor??2.5,currentIntervalDays:o.interval_days??0,currentRepetitions:o.repetitions??0,nowEpoch:Math.floor(Date.now()/1e3)});F(c,o.id,E)}if(o.concept_name&&!D){let E=c.query("SELECT category FROM vl_concepts WHERE concept_name = ? LIMIT 1").get(o.concept_name),p=U(c,{conceptName:o.concept_name,category:E?.category??"general",isCorrect:f});if(p.promoted?console.log(`  \u{1F389} Level up! ${o.concept_name}: ${p.previousLevel} \u2192 ${p.profile.current_level}
`):p.demoted&&console.log(`  \u{1F4C9} Level dropped: ${o.concept_name}: ${p.previousLevel} \u2192 ${p.profile.current_level}
`),Q(o,f,p.profile.current_level,y)){let w=j(o);_.splice(m+1,0,w),y.add(w.id),console.log(`  \u2795 Follow-up added: ${w.question.slice(0,60)}...
`)}}$(c,f)}catch{}m++}l.close(),c.close();let S=u>0?Math.round(a/u*100):0;console.log(`
\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501`),console.log(`  Quiz complete: ${a}/${u} correct (${S}%)`),console.log(`\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
`)}async function le(e){let t="http://localhost:37778";async function r(i,a){try{let u=await fetch(`${t}${i}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a),signal:AbortSignal.timeout(12e4)}),_=await u.json().catch(()=>({}));return{ok:u.ok,status:u.status,data:_}}catch(u){return{ok:!1,status:0,data:{error:u.message}}}}let s=e;if(!s){let i=q();if(!i)return;let a=i.query(`
    ...
    `).get()??i.query("SELECT content_session_id, project, started_at FROM sdk_sessions ORDER BY started_at_epoch DESC LIMIT 1").get();if(i.close(),!a){console.log(`
...
`)}}_e().catch(e=>{console.error("Error:",e.message),process.exit(1)});

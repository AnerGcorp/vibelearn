...
Email Investigation Batch Processor.

## Overview

Ragtime processes email corpus files through Claude for entity/relationship/timeline extraction. Each file gets a NEW session — context is managed per-session, not by conversation continuation.
...
| `RAGTIME_PLUGIN_PATH` | `./plugin` | Path to vibelearn plugin |
| `VIBELEARN_WORKER_PORT` | `37778` | Worker service port |
...
1. **Startup**: Sets `VIBELEARN_MODE=email-investigation` and cleans up old transcripts
2. **Processing**: For each file:
   ...
   - VibeLearn worker processes and stores observations for the session
...
The main vibelearn repository is licensed under AGPL 3.0, but ragtime uses the more restrictive PolyForm Noncommercial license to ensure it remains freely available for personal and educational use while preventing commercial exploitation.

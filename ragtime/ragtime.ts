 ...
 * Each file gets a NEW session - context is managed by VibeLearn's context
...
const CONFIG = {
  ...
  // Path to vibelearn plugin
  pluginPath: process.env.RAGTIME_PLUGIN_PATH ||
    path.join(process.cwd(), "plugin"),

  // Worker port
  workerPort: parseInt(process.env.VIBELEARN_WORKER_PORT || "37777", 10),
...
// Set email-investigation mode for VibeLearn
process.env.VIBELEARN_MODE = "email-investigation";
 ...
 * Context is injected by VibeLearn hooks, not conversation continuation
 */

...
const MARKETPLACE_DIR = join(homedir(), '.claude', 'plugins', 'marketplaces', 'anergcorp');
...
function registerPlugin(version: string): void {
  ...
  const pluginCachePath = join(PLUGINS_DIR, 'cache', 'anergcorp', 'vibelearn', version);
  const now = new Date().toISOString();

  installedPlugins.plugins['vibelearn@anergcorp'] = [
    {
      scope: 'user',
...
function enablePluginInClaudeSettings(): void {
  ...
  settings.enabledPlugins['vibelearn@anergcorp'] = true;
...
export async function runInstallation(selectedIDEs: IDE[]): Promise<void> {
  const tempDir = join(tmpdir(), `vibelearn-install-${Date.now()}`);
  ...
  if (selectedIDEs.includes('cursor')) {
    ...
    p.log.info('Run: vibelearn cursor-setup (coming soon)');
  }
}

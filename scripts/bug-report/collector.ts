...
async function getVibeLearnVersion(): Promise<string> {
  try {
...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "vibelearn.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".vibelearn");
  ...
  const isDevMode = cwd.includes("vibelearn") && !cwd.includes(".claude");

  // Collect version information
  const [vibelearn, claudeCode, bun, osVersion] = await Promise.all([
    getVibeLearnVersion(),
  ...
  const workerPort = pidInfo?.port || 37778;
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "vibelearn.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **VibeLearn**: ${diagnostics.versions.vibelearn}\n`;

# OpenClaw VibeLearn Plugin — Testing Guide
...
4. Starts a mock vibelearn worker on port 37778
...
node openclaw.mjs plugins info vibelearn
node openclaw.mjs plugins doctor
```

**Expected:**
- `vibelearn` appears in the plugins list as "enabled" or "loaded"
- Info shows version 1.0.0, source at `/home/node/.openclaw/extensions/vibelearn/`
- Doctor reports no issues

#### 2. Inspect plugin files

```bash
ls -la /home/node/.openclaw/extensions/vibelearn/
cat /home/node/.openclaw/extensions/vibelearn/openclaw.plugin.json
cat /home/node/.openclaw/extensions/vibelearn/package.json
```

**Expected:**
- `dist/index.js` exists (compiled plugin)
- `openclaw.plugin.json` has `"id": "vibelearn"` and `"kind": "memory"`
...
curl -s --max-time 3 http://localhost:37778/stream
...
- `[vibelearn] OpenClaw plugin loaded — v1.0.0`
- `[vibelearn] Observation feed starting — channel: telegram, target: test-chat-id-12345`
- `[vibelearn] Connecting to SSE stream at http://localhost:37778/stream`
- `[vibelearn] Connected to SSE stream`
...
For testing with a real vibelearn worker and real messaging channel:

### Prerequisites

- OpenClaw gateway installed and configured
- VibeLearn worker running on port 37778
...
# Enable it
openclaw plugins enable vibelearn
```
...
Start a Claude Code session with vibelearn enabled and perform any action. The worker will emit a `new_observation` SSE event.
...
- **Fix:** Start the worker: `cd /path/to/vibelearn && npm run build-and-sync`

### Port mismatch
- **Fix:** Ensure `workerPort` in config matches the worker's actual port (default: 37778)
...
- **Fix:** Add `"slots": { "memory": "vibelearn" }` to plugins config

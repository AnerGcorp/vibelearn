...
test_configure_new_config() {
  ...
  assert_eq "vibelearn" "$memory_slot" "Memory slot set to vibelearn in new config"

  local enabled
  enabled="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].enabled);")"
  assert_eq "true" "$enabled" "vibelearn entry is enabled in new config"

  local worker_port
  worker_port="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].config.workerPort);")"
...
test_configure_existing_config() {
  ...
  local memory_slot
  memory_slot="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.slots.memory);")"
  assert_eq "vibelearn" "$memory_slot" "Memory slot updated from memory-core to vibelearn"
  ...
  # Verify vibelearn entry was added
  local cm_enabled
  cm_enabled="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].enabled);")"
  assert_eq "true" "$cm_enabled" "vibelearn entry added and enabled"
...
# Test: configure_memory_slot() — preserves existing vibelearn config
test_configure_preserves_existing_cm_config() {
  ...
  local cm_enabled
  cm_enabled="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].enabled);")"
  assert_eq "true" "$cm_enabled" "vibelearn entry enabled when previously disabled"

  local custom_port
  custom_port="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].config.workerPort);")"
  assert_eq "38888" "$custom_port" "Existing custom workerPort preserved"

  local feed_channel
  feed_channel="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].config.observationFeed.channel);")"
...
# Verify the VIBELEARN_REPO constant
assert_contains "$VIBELEARN_REPO" "github.com/anergcorp/vibelearn" "VIBELEARN_REPO points to correct repository"
...
test_write_settings_new_file() {
  ...
  local settings_file="${fake_home}/.vibelearn/settings.json"
  assert_file_exists "$settings_file" "settings.json created at ~/.vibelearn/settings.json"

  # Verify it's valid JSON with expected defaults
  local provider
  provider="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_PROVIDER);")"
  assert_eq "claude" "$provider" "VIBELEARN_PROVIDER set to claude"

  local auth_method
  auth_method="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_CLAUDE_AUTH_METHOD);")"
  assert_eq "cli" "$auth_method" "VIBELEARN_CLAUDE_AUTH_METHOD set to cli for Claude provider"

  local worker_port
  worker_port="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_WORKER_PORT);")"
  assert_eq "37777" "$worker_port" "VIBELEARN_WORKER_PORT defaults to 37777"

  local model
  model="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_MODEL);")"
  assert_eq "claude-sonnet-4-5" "$model" "VIBELEARN_MODEL defaults to claude-sonnet-4-5"
...
test_write_settings_gemini() {
  ...
  local provider
  provider="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_PROVIDER);")"
  assert_eq "gemini" "$provider" "Gemini: VIBELEARN_PROVIDER set to gemini"

  local api_key
  api_key="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_GEMINI_API_KEY);")"
  assert_eq "test-gemini-key-1234" "$api_key" "Gemini: API key stored in settings"

  local gemini_model
  gemini_model="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_GEMINI_MODEL);")"
...
test_write_settings_openrouter() {
  ...
  local provider
  provider="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_PROVIDER);")"
  assert_eq "openrouter" "$provider" "OpenRouter: VIBELEARN_PROVIDER set to openrouter"

  local api_key
  api_key="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_OPENROUTER_API_KEY);")"
  assert_eq "sk-or-test-key-5678" "$api_key" "OpenRouter: API key stored in settings"

  local or_model
  or_model="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_OPENROUTER_MODEL);")"
...
test_write_settings_preserves_existing() {
  ...
  mkdir -p "${fake_home}/.vibelearn"
  local settings_file="${fake_home}/.vibelearn/settings.json"
  node -e "
    const settings = {
      VIBELEARN_PROVIDER: 'gemini',
      VIBELEARN_GEMINI_API_KEY: 'old-key',
      VIBELEARN_WORKER_PORT: '38888',
      VIBELEARN_LOG_LEVEL: 'DEBUG'
    };
  ...
  local custom_port
  custom_port="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_WORKER_PORT);")"
  assert_eq "38888" "$custom_port" "Preserve: existing custom WORKER_PORT preserved"

  # Custom log level should be preserved
  local log_level
  log_level="$(node -e "const s = JSON.parse(require('fs').readFileSync('${settings_file}','utf8')); console.log(s.VIBELEARN_LOG_LEVEL);")"
...
###############################################################################
# Test: find_vibelearn_install_dir() — not found scenario
###############################################################################

echo ""
echo "=== find_vibelearn_install_dir() ==="

test_find_install_dir_not_found() {
  local fake_home
  fake_home="$(mktemp -d)"
  HOME="$fake_home"
  VIBELEARN_INSTALL_DIR=""

  if find_vibelearn_install_dir 2>/dev/null; then
    test_fail "find_vibelearn_install_dir should return 1 when not found"
  else
    test_pass "find_vibelearn_install_dir returns 1 when not found"
  fi

  assert_eq "" "$VIBELEARN_INSTALL_DIR" "VIBELEARN_INSTALL_DIR is empty when not found"
...
# Test: find_vibelearn_install_dir() — found in ~/.openclaw/extensions/vibelearn/
test_find_install_dir_openclaw_extensions() {
  ...
  mkdir -p "${fake_home}/.openclaw/extensions/vibelearn/plugin/scripts"
  touch "${fake_home}/.openclaw/extensions/vibelearn/plugin/scripts/worker-service.cjs"

  if find_vibelearn_install_dir 2>/dev/null; then
    test_pass "find_vibelearn_install_dir finds dir in ~/.openclaw/extensions/vibelearn/"
    assert_eq "${fake_home}/.openclaw/extensions/vibelearn" "$VIBELEARN_INSTALL_DIR" "VIBELEARN_INSTALL_DIR set correctly for openclaw extensions"
  else
    test_fail "find_vibelearn_install_dir should find dir in ~/.openclaw/extensions/vibelearn/"
  fi
...
# Test: find_vibelearn_install_dir() — found in ~/.claude/plugins/marketplaces/anergcorp/
test_find_install_dir_marketplace() {
  ...
  mkdir -p "${fake_home}/.claude/plugins/marketplaces/anergcorp/plugin/scripts"
  touch "${fake_home}/.claude/plugins/marketplaces/anergcorp/plugin/scripts/worker-service.cjs"

  if find_vibelearn_install_dir 2>/dev/null; then
    test_pass "find_vibelearn_install_dir finds dir in marketplace path"
    assert_eq "${fake_home}/.claude/plugins/marketplaces/anergcorp" "$VIBELEARN_INSTALL_DIR" "VIBELEARN_INSTALL_DIR set correctly for marketplace"
  else
    test_fail "find_vibelearn_install_dir should find dir in marketplace path"
  fi
...
test_start_worker_no_install_dir() {
  ...
  assert_contains "$output" "Cannot find vibelearn plugin installation directory" "start_worker error message mentions install dir"
...
test_print_completion_summary() {
  ...
  assert_contains "$output" "/vibelearn-status" "Completion summary mentions status command"
...
for fn in find_vibelearn_install_dir start_worker verify_health print_completion_summary; do
  if declare -f "$fn" &>/dev/null; then
    test_pass "Function ${fn}() is defined"
  else
    test_fail "Function ${fn}() should be defined"
...
test_write_observation_feed_config_writes_json() {
  ...
  # Create an existing openclaw.json with vibelearn entry
  mkdir -p "${fake_home}/.openclaw"
  local config_file="${fake_home}/.openclaw/openclaw.json"
  node -e "
    const config = {
      plugins: {
        slots: { memory: 'vibelearn' },
        entries: {
          'vibelearn': {
            enabled: true,
  ...
  local feed_enabled
  feed_enabled="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].config.observationFeed.enabled);")"
  ...
  local feed_to
  feed_to="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.entries['vibelearn'].config.observationFeed.to);")"
...
test_write_feed_config_node_path() {
  ...
  result="$(bash -c '
    ...
    node -e "
      ...
      if (!config.plugins.entries[\"vibelearn\"]) {
        config.plugins.entries[\"vibelearn\"] = { enabled: true, config: {} };
      }
      if (!config.plugins.entries[\"vibelearn\"].config) {
        config.plugins.entries[\"vibelearn\"].config = {};
      }

      config.plugins.entries[\"vibelearn\"].config.observationFeed = {
        enabled: true,
        channel: channel,
        to: targetId
      };
...
test_completion_summary_with_feed() {
  ...
  assert_contains "$output" "/vibelearn-feed" "Summary includes feed check command when configured"
...
test_completion_summary_without_feed() {
  ...
  assert_contains "$output" "/vibelearn-status" "Summary includes status check command"
...
###############################################################################
# Test: is_vibelearn_installed() — upgrade detection
###############################################################################

echo ""
echo "=== is_vibelearn_installed() ==="

test_is_vibelearn_installed_found() {
  local fake_home
  fake_home="$(mktemp -d)"
  HOME="$fake_home"
  VIBELEARN_INSTALL_DIR=""
  ...
  if is_vibelearn_installed; then
    test_pass "is_vibelearn_installed returns true when plugin exists"
  else
    test_fail "is_vibelearn_installed should return true when plugin exists"
  fi
...
test_is_vibelearn_installed_not_found() {
  local fake_home
  fake_home="$(mktemp -d)"
  HOME="$fake_home"
  VIBELEARN_INSTALL_DIR=""
  ...
  if is_vibelearn_installed; then
    test_fail "is_vibelearn_installed should return false when plugin not found"
  else
    test_pass "is_vibelearn_installed returns false when plugin not found"
  fi
...
test_main_calls_is_vibelearn_installed() {
  if grep -q 'is_vibelearn_installed' "$INSTALL_SCRIPT"; then
    test_pass "main() calls is_vibelearn_installed for upgrade detection"
  else
    test_fail "main() should call is_vibelearn_installed"
  fi
}
...
test_install_sh_has_stable_url_in_usage() {
  if grep -q 'raw.githubusercontent.com/anergcorp/vibelearn/main/openclaw/install.sh' "$INSTALL_SCRIPT"; then

...
# vibelearn OpenClaw Plugin Installer
# Installs the vibelearn plugin for OpenClaw gateways.
#
# Usage:
#   curl -fsSL https://install.vibelearn.dev/openclaw.sh | bash
#   # Or with options:
#   curl -fsSL https://install.vibelearn.dev/openclaw.sh | bash -s -- --provider=gemini --api-key=YOUR_KEY
#   # Direct execution:
...
###############################################################################
# Upgrade detection — check if vibelearn is already installed
###############################################################################

is_vibelearn_installed() {
...
check_openclaw() {
  if ! find_openclaw; then
    ...
    error ""
    error "The vibelearn plugin requires an OpenClaw gateway to be installed."
...
VIBELEARN_REPO="https://github.com/anergcorp/vibelearn.git"
VIBELEARN_BRANCH="${CLI_BRANCH:-main}"
...
resolve_extension_dir() {
  ...
  if [[ -f "$oc_config" ]] && command -v node &>/dev/null; then
    ...
    existing_path="$(node -e "
      try {
        ...
        const p = c?.plugins?.installs?.['vibelearn']?.installPath;
        if (p) console.log(p);
      } catch {}
    " 2>/dev/null)" || true
    ...
    load_path="$(node -e "
      try {
        ...
        const p = paths.find(p => p.endsWith('/vibelearn'));
        if (p) console.log(p);
      } catch {}
    " 2>/dev/null)" || true
    if [[ -n "$load_path" ]]; then
  ...
  echo "${HOME}/.openclaw/extensions/vibelearn"
}

VIBELEARN_EXTENSION_DIR=""

install_plugin() {
  # Check for git before attempting clone
  check_git

  VIBELEARN_EXTENSION_DIR="$(resolve_extension_dir)"

  # Remove existing plugin installation to allow clean re-install
  local existing_plugin_dir="$VIBELEARN_EXTENSION_DIR"
  if [[ -d "$existing_plugin_dir" ]]; then
    info "Removing existing vibelearn plugin at ${existing_plugin_dir}..."
  ...
  info "Cloning vibelearn repository (branch: ${VIBELEARN_BRANCH})..."
  if ! git clone --depth 1 --branch "$VIBELEARN_BRANCH" "$VIBELEARN_REPO" "$build_dir/vibelearn" 2>&1; then
    error "Failed to clone vibelearn repository"
    error "Check your internet connection and try again."
    exit 1
  fi

  local plugin_src="${build_dir}/vibelearn/openclaw"
  ...
  # Clean up stale vibelearn plugin entry before installing.
  # If the config references vibelearn but the plugin isn't installed,
  ...
  if [[ -f "$oc_config" ]]; then
    saved_plugin_config=$(INSTALLER_CONFIG_FILE="$oc_config" node -e "
      ...
      const entry = config?.plugins?.entries?.['vibelearn'];
      if (entry || config?.plugins?.slots?.memory === 'vibelearn') {
        ...
        if (entry) delete config.plugins.entries['vibelearn'];
        ...
        if (config?.plugins?.slots?.memory === 'vibelearn') {
  ...
  info "Installing vibelearn plugin into OpenClaw..."
  if ! run_openclaw plugins install "$installable_dir" 2>&1; then
    error "Failed to install vibelearn plugin"
    error "Try manually: ${OPENCLAW_PATH} plugins install <path>"
    exit 1
  fi

  # Enable the plugin
  info "Enabling vibelearn plugin..."
  if ! run_openclaw plugins enable vibelearn 2>&1; then
    error "Failed to enable vibelearn plugin"
    error "Try manually: ${OPENCLAW_PATH} plugins enable vibelearn"
    exit 1
  fi
  ...
  if [[ -n "$saved_plugin_config" && "$saved_plugin_config" != "{}" ]]; then
    ...
    INSTALLER_CONFIG_FILE="$oc_config" INSTALLER_SAVED_CONFIG="$saved_plugin_config" node -e "
      ...
      if (config?.plugins?.entries?.['vibelearn']) {
        config.plugins.entries['vibelearn'].config = savedConfig;
        fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
      }
    " 2>/dev/null || warn "Could not restore previous plugin config — configure manually"
  fi

  success "vibelearn plugin installed and enabled"
  ...
  # of the main repo. We copy them so find_vibelearn_install_dir() can locate
  # the worker-service.cjs and the worker runs the updated version.
  local extension_dir="$VIBELEARN_EXTENSION_DIR"
  local repo_root="${build_dir}/vibelearn"
...
# Sets plugins.slots.memory = "vibelearn" in ~/.openclaw/openclaw.json
###############################################################################
...
configure_memory_slot() {
  ...
  if [[ ! -f "$config_file" ]]; then
    ...
    info "Creating OpenClaw configuration with vibelearn memory slot..."
    ...
    "
    success "Created ${config_file} with memory slot set to vibelearn"
    return 0
  fi

  # Config file exists — update it to set the memory slot
  info "Updating OpenClaw configuration to use vibelearn memory slot..."
  ...
  INSTALLER_CONFIG_FILE="$config_file" node -e "
    ...
    // Set memory slot to vibelearn
    config.plugins.slots.memory = 'vibelearn';

    // Ensure vibelearn entry exists and is enabled
    if (!config.plugins.entries['vibelearn']) {
      config.plugins.entries['vibelearn'] = {
        enabled: true,
        config: {
          workerPort: 37777,
          syncMemoryFile: true
      ...
      config.plugins.entries['vibelearn'].enabled = true;
      // Remove unrecognized keys that cause OpenClaw config validation errors
      const allowedKeys = new Set(['enabled', 'config']);
      for (const key of Object.keys(config.plugins.entries['vibelearn'])) {
        if (!allowedKeys.has(key)) {
          delete config.plugins.entries['vibelearn'][key];
        }
      }
    }
  ...
  success "Memory slot set to vibelearn in ${config_file}"
}
...
setup_ai_provider() {
    ...
    case "$CLI_PROVIDER" in
      ...
      gemini)
        ...
        if [[ -n "$AI_PROVIDER_API_KEY" ]]; then
          ...
          warn "Selected via --provider: Gemini (no API key — add later in ~/.vibelearn/settings.json)"
        fi
        ;;
      openrouter)
        ...
        if [[ -n "$AI_PROVIDER_API_KEY" ]]; then
          ...
          warn "Selected via --provider: OpenRouter (no API key — add later in ~/.vibelearn/settings.json)"
        fi
        ;;
      *)
  ...
  echo -e "  Choose your AI provider for vibelearn:"
  echo ""
    ...
    case "$choice" in
      ...
      2)
        ...
        if [[ -z "$AI_PROVIDER_API_KEY" ]]; then
          warn "No API key provided — you can add it later in ~/.vibelearn/settings.json"
        else
...
###############################################################################
# Write settings.json — creates ~/.vibelearn/settings.json with all defaults
# Schema: flat key-value (not nested { env: {...} })
# Defaults sourced from SettingsDefaultsManager.ts
###############################################################################

write_settings() {
  local settings_dir="${HOME}/.vibelearn"
  ...
  node -e "
    ...
    const defaults = {
      ...
      VIBELEARN_DATA_DIR: path.join(homedir, '.vibelearn'),
      ...
      VIBELEARN_CONTEXT_OBSERVATION_TYPES: 'bugfix,feature,refactor,discovery,decision,change',
      VIBELEARN_CONTEXT_OBSERVATION_CONCEPTS: 'how-it-works,why-it-exists,what-changed,problem-solution,gotcha,pattern,trade-off',
      VIBELEARN_CONTEXT_FULL_COUNT: '5',
      VIBELEARN_CONTEXT_FULL_FIELD: 'narrative',
      VIBELEARN_CONTEXT_SESSION_COUNT: '10',
      VIBELEARN_CONTEXT_SHOW_LAST_SUMMARY: 'true',
      VIBELEARN_CONTEXT_SHOW_LAST_MESSAGE: 'false',
    ...
    const overrides = { VIBELEARN_PROVIDER: provider };
    if (provider === 'claude') {
      overrides.VIBELEARN_CLAUDE_AUTH_METHOD = 'cli';
    } else if (provider === 'gemini') {
      overrides.VIBELEARN_GEMINI_API_KEY = apiKey;
      overrides.VIBELEARN_GEMINI_MODEL = 'gemini-2.5-flash-lite';
    } else if (provider === 'openrouter') {
      overrides.VIBELEARN_OPENROUTER_API_KEY = apiKey;
      overrides.VIBELEARN_OPENROUTER_MODEL = 'xiaomi/mimo-v2-flash:free';
    }
...
###############################################################################
# Locate the installed vibelearn plugin directory
# Checks common OpenClaw and Claude Code plugin install paths
###############################################################################

VIBELEARN_INSTALL_DIR=""

find_vibelearn_install_dir() {
  local resolved_dir
  resolved_dir="$(resolve_extension_dir)"
  local -a search_paths=(
    "$resolved_dir"
    "${HOME}/.openclaw/extensions/vibelearn"
    "${HOME}/.claude/plugins/marketplaces/anergcorp"
    "${HOME}/.openclaw/plugins/vibelearn"
  )

  for candidate in "${search_paths[@]}"; do
    if [[ -f "${candidate}/plugin/scripts/worker-service.cjs" ]]; then
      VIBELEARN_INSTALL_DIR="$candidate"
      return 0
    fi
  done
  ...
  for root in "${roots[@]}"; do
    if [[ -d "$root" ]]; then
      ...
      if [[ -n "$found" ]]; then
        ...
        VIBELEARN_INSTALL_DIR="${found%/plugin/scripts/worker-service.cjs}"
        return 0
      fi
    fi
  done
...
# Starts the vibelearn worker using bun in the background
###############################################################################
...
start_worker() {
  info "Starting vibelearn worker service..."

  if ! find_vibelearn_install_dir; then
    error "Cannot find vibelearn plugin installation directory"
    error "Expected worker-service.cjs in one of:"
    error "  ~/.openclaw/extensions/vibelearn/plugin/scripts/"
    error "  ~/.claude/plugins/marketplaces/anergcorp/plugin/scripts/"
    error ""
    error "Try reinstalling the plugin and re-running this installer."
    return 1
  fi

  local worker_script="${VIBELEARN_INSTALL_DIR}/plugin/scripts/worker-service.cjs"
  local log_dir="${HOME}/.vibelearn/logs"
  local log_date
  log_date="$(date +%Y-%m-%d)"
  ...
  VIBELEARN_WORKER_PORT=37777 nohup "$BUN_PATH" "$worker_script" \
    >> "$log_file" 2>&1 &
  WORKER_PID=$!

  # Write PID file for future management
  local pid_file="${HOME}/.vibelearn/worker.pid"
  mkdir -p "${HOME}/.vibelearn"
...
verify_health() {
  ...
  if [[ "$health_alive" != "true" ]]; then
    ...
    warn "  Or check logs: ~/.vibelearn/logs/"
    return 1
  fi
...
setup_observation_feed() {
  ...
  echo ""
  echo "  vibelearn can stream AI-compressed observations to a messaging"
  ...
  if [[ "$NON_INTERACTIVE" == "true" ]]; then
    ...
    info "  plugins.entries.vibelearn.config.observationFeed"
    return 0
  fi
...
write_observation_feed_config() {
  ...
  if command -v jq &>/dev/null; then
    ...
    jq --arg channel "$FEED_CHANNEL" --arg target "$FEED_TARGET_ID" '
      .plugins //= {} |
      .plugins.entries //= {} |
      .plugins.entries['vibelearn'] //= {"enabled": true, "config": {}} |
      .plugins.entries['vibelearn'].config //= {} |
      .plugins.entries['vibelearn'].config.observationFeed = {
        "enabled": true,
        "channel": $channel,
        "to": $target
      }
...
config['plugins']['entries'].setdefault('vibelearn', {'enabled': True, 'config': {}})
config['plugins']['entries']['vibelearn'].setdefault('config', {})
...
"
  ...
  echo "  [vibelearn] Observation feed starting — channel: ${FEED_CHANNEL}, target: ${FEED_TARGET_ID}"
  echo ""
  info "After restarting, run /vibelearn-feed in any OpenClaw chat to verify"
...
print_completion_summary() {
  ...
  if [[ -n "$WORKER_VERSION" ]]; then
    echo -e "  ${COLOR_GREEN}✓${COLOR_RESET}  vibelearn v${COLOR_BOLD}${WORKER_VERSION}${COLOR_RESET} installed and running"
  else
    echo -e "  ${COLOR_GREEN}✓${COLOR_RESET}  vibelearn plugin installed and enabled"
  fi
  ...
  echo -e "  ${COLOR_GREEN}✓${COLOR_RESET}  Settings written to ~/.vibelearn/settings.json"
  ...
  if [[ -n "$WORKER_PID" ]] && kill -0 "$WORKER_PID" 2>/dev/null; then
    ...
    echo -e "  ${COLOR_YELLOW}⚠${COLOR_RESET}  Worker may not be running — check logs at ~/.vibelearn/logs/"
  fi
  ...
  if [[ "$FEED_CONFIGURED" == "true" ]]; then
    ...
    echo -e "     plugins.entries.vibelearn.config.observationFeed"
  fi
  ...
  echo -e "  ${COLOR_CYAN}2.${COLOR_RESET} Verify with ${COLOR_BOLD}/vibelearn-status${COLOR_RESET} in any OpenClaw chat"
  echo -e "  ${COLOR_CYAN}3.${COLOR_RESET} Check the viewer UI at ${COLOR_BOLD}http://localhost:37777${COLOR_RESET}"
  if [[ "$FEED_CONFIGURED" == "true" ]]; then
    echo -e "  ${COLOR_CYAN}4.${COLOR_RESET} Run ${COLOR_BOLD}/vibelearn-feed${COLOR_RESET} to check feed status"
  fi
  echo ""
  echo -e "  ${COLOR_BOLD}To re-run this installer:${COLOR_RESET}"
  echo "  bash <(curl -fsSL https://install.vibelearn.dev/openclaw.sh)"
  echo ""
}
...
main() {
  ...
  echo ""
  info "${COLOR_BOLD}[3/8]${COLOR_RESET} Installing vibelearn plugin..."

  if [[ "$UPGRADE_MODE" == "true" ]] && is_vibelearn_installed; then
    success "vibelearn already installed at ${VIBELEARN_INSTALL_DIR}"
  ...
  if check_port_37777; then
    ...
    if verify_health; then
      ...
      if [[ -n "$VIBELEARN_INSTALL_DIR" ]] || find_vibelearn_install_dir; then
        expected_version="$(INSTALLER_PKG="${VIBELEARN_INSTALL_DIR}/package.json" node -e "
      ...
      warn "or change VIBELEARN_WORKER_PORT in ~/.vibelearn/settings.json"
    fi
  else
    if start_worker; then
      verify_health || true
      ...
      warn "  cd ~/.openclaw/extensions/vibelearn && bun plugin/scripts/worker-service.cjs"
    fi
  fi

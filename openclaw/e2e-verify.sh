#!/usr/bin/env bash
# e2e-verify.sh — Automated E2E verification for vibelearn plugin on OpenClaw
#
...
if echo "$PLUGIN_LIST" | grep -q "vibelearn"; then
...
# Check plugin info
PLUGIN_INFO=$(node /app/openclaw.mjs plugins info vibelearn 2>&1 || true)
if echo "$PLUGIN_INFO" | grep -qi "vibelearn"; then
  pass "Plugin info shows vibelearn details"
else
...
if echo "$PLUGIN_LIST" | grep -A1 "vibelearn" | grep -qi "enabled\|loaded"; then
...
if [ ! -d "$EXTENSIONS_DIR" ]; then
  ...
  EXTENSIONS_DIR="/home/node/.openclaw/extensions/vibelearn"
  if [ ! -d "$EXTENSIONS_DIR" ]; then
    # Search for it
...
echo "  Starting mock vibelearn worker..."
...
# The memory slot must be set to "vibelearn" to match what `plugins install` configured.
...
if grep -qi "vibelearn" "$GATEWAY_LOG" 2>/dev/null; then
  pass "Gateway log mentions vibelearn plugin"
else
  fail "Gateway log does not mention vibelearn"

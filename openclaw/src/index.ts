...
interface VibeLearnPluginConfig {
...
async function workerPost(
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn(`[vibelearn] Worker POST ${path} returned ${response.status}`);
      return null;
    }
    return (await response.json()) as Record<string, unknown>;
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    logger.warn(`[vibelearn] Worker POST ${path} failed: ${message}`);
    return null;
  }
}
...
async function workerGetText(
  ...
  try {
    const response = await fetch(`${workerBaseUrl(port)}${path}`);
    if (!response.ok) {
      logger.warn(`[vibelearn] Worker GET ${path} returned ${response.status}`);
      return null;
    }
    return await response.text();
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    logger.warn(`[vibelearn] Worker GET ${path} failed: ${message}`);
    return null;
  }
}
...
async function workerGetJson(
  ...
  try {
    ...
    logger.warn(`[vibelearn] Worker GET ${path} returned non-JSON response`);
    return null;
  }
}
...
async function sendDirectTelegram(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn(`[vibelearn] Direct Telegram send failed (${response.status}): ${body}`);
    }
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    logger.warn(`[vibelearn] Direct Telegram send error: ${message}`);
  }
}
...
function sendToChannel(
  ...
  if (!mapping) {
    api.logger.warn(`[vibelearn] Unsupported channel type: ${channel}`);
    return Promise.resolve();
  }

  const channelApi = api.runtime.channel[mapping.namespace];
  if (!channelApi) {
    api.logger.warn(`[vibelearn] Channel "${channel}" not available in runtime`);
    return Promise.resolve();
  }

  const senderFunction = channelApi[mapping.functionName];
  if (!senderFunction) {
    api.logger.warn(`[vibelearn] Channel "${channel}" has no ${mapping.functionName} function`);
    return Promise.resolve();
  }
  ...
  return senderFunction(...args).catch((error: unknown) => {
    const message = error instanceof Error ? error.message : String(error);
    api.logger.error(`[vibelearn] Failed to send to ${channel}: ${message}`);
  });
}
...
async function connectToSSEStream(
  ...
  while (!abortController.signal.aborted) {
    try {
      ...
      api.logger.info(`[vibelearn] Connecting to SSE stream at ${workerBaseUrl(port)}/stream`);
      ...
      backoffMs = 1000;
      api.logger.info("[vibelearn] Connected to SSE stream");
      ...
      while (true) {
        ...
        if (buffer.length > MAX_SSE_BUFFER_SIZE) {
          api.logger.warn("[vibelearn] SSE buffer overflow, clearing buffer");
          buffer = "";
        }
        ...
        for (const frame of frames) {
          ...
          try {
            ...
            api.logger.warn(`[vibelearn] Failed to parse SSE frame: ${errorMessage}`);
          }
        }
      }
    } catch (error: unknown) {
      ...
      api.logger.warn(`[vibelearn] SSE stream error: ${errorMessage}. Reconnecting in ${backoffMs / 1000}s`);
    }
...
export default function vibeLearnPlugin(api: OpenClawPluginApi): void {
  const userConfig = (api.pluginConfig || {}) as VibeLearnPluginConfig;
  ...
  // ------------------------------------------------------------------
  // Event: session_start — init vibelearn session (fires on /new, /reset)
  // ------------------------------------------------------------------
  api.on("session_start", async (_event, ctx) => {
    ...
    api.logger.info(`[vibelearn] Session initialized: ${contentSessionId}`);
  });
  ...
  api.on("after_compaction", async (_event, ctx) => {
    ...
    api.logger.info(`[vibelearn] Session re-initialized after compaction: ${contentSessionId}`);
  });
  ...
  api.on("before_prompt_build", async (_event, ctx) => {
    ...
    if (contextText) {
      api.logger.info(`[vibelearn] Context injected via system prompt for agent=${ctx.agentId ?? "unknown"}`);
  ...
  api.on("tool_result_persist", (event, ctx) => {
    api.logger.info(`[vibelearn] tool_result_persist fired: tool=${event.toolName ?? "unknown"} agent=${ctx.agentId ?? "none"} session=${ctx.sessionKey ?? "none"}`);
  ...
  api.on("gateway_start", async () => {
    sessionIds.clear();
    contextCache.clear();
    api.logger.info("[vibelearn] Gateway started — session tracking reset");
  });
  ...
  api.registerService({
    ...
    start: async (_ctx) => {
      ...
      if (!feedConfig?.enabled) {
        api.logger.info("[vibelearn] Observation feed disabled");
        return;
      }

      if (!feedConfig.channel || !feedConfig.to) {
        api.logger.warn("[vibelearn] Observation feed misconfigured — channel or target missing");
        return;
      }

      api.logger.info(`[vibelearn] Observation feed starting — channel: ${feedConfig.channel}, target: ${feedConfig.to}`);
    ...
    stop: async (_ctx) => {
      ...
      api.logger.info("[vibelearn] Observation feed stopped — SSE connection closed");
    },
  });
  ...
  // ------------------------------------------------------------------
  // Command: /vibelearn_feed — status & toggle
  // ------------------------------------------------------------------
  api.registerCommand({
    name: "vibelearn_feed",
    description: "Show or toggle VibeLearn observation feed status",
    acceptsArgs: true,
    handler: async (ctx) => {
      const feedConfig = userConfig.observationFeed;
  ...
  // ------------------------------------------------------------------
  // Command: /vibelearn-search — query worker search API
  // Usage: /vibelearn-search <query> [limit]
  // ------------------------------------------------------------------
  api.registerCommand({
    name: "vibelearn-search",
    description: "Search VibeLearn observations by query",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      if (!raw) {
        return "Usage: /vibelearn-search <query> [limit]";
      }
      ...
      if (!data) {
        return "VibeLearn search failed (worker unavailable or invalid response).";
      }

      const items = Array.isArray(data.items) ? data.items : [];
      return [
        `VibeLearn Search: \"${query}\"`,
  ...
  // ------------------------------------------------------------------
  // Command: /vibelearn-recent — recent context snapshot
  // Usage: /vibelearn-recent [project] [limit]
  // ------------------------------------------------------------------
  api.registerCommand({
    name: "vibelearn-recent",
    description: "Show recent VibeLearn context for a project",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      ...
      if (!data) {
        return "VibeLearn recent context failed (worker unavailable or invalid response).";
      }
  ...
  // ------------------------------------------------------------------
  // Command: /vibelearn-timeline — search and timeline around best match
  // Usage: /vibelearn-timeline <query> [depthBefore] [depthAfter]
  // ------------------------------------------------------------------
  api.registerCommand({
    name: "vibelearn-timeline",
    description: "Find best memory match and show nearby timeline events",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      if (!raw) {
        return "Usage: /vibelearn-timeline <query> [depthBefore] [depthAfter]";
      }
      ...
      if (!data) {
        return "VibeLearn timeline lookup failed (worker unavailable or invalid response).";
      }
  ...
  // ------------------------------------------------------------------
  // Command: /vibelearn_status — worker health check
  // ------------------------------------------------------------------
  api.registerCommand({
    name: "vibelearn_status",
    description: "Check VibeLearn worker health and session status",
    handler: async () => {
      const healthText = await workerGetText(workerPort, "/api/health", api.logger);
      if (!healthText) {
        return { text: `VibeLearn worker unreachable at port ${workerPort}` };
      }

      try {
        const health = JSON.parse(healthText);
        return { text: [
          "VibeLearn Worker Status",
        ...
        return { text: `VibeLearn worker responded but returned unexpected data` };
      }
    },
  });

  api.logger.info(`[vibelearn] VibeLearn plugin loaded — v1.0.0 (worker: 127.0.0.1:${workerPort})`);
}

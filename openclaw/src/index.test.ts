...
import vibeLearnPlugin from "./index.js";
...
function createMockApi(pluginConfigOverride: Record<string, any> = {}) {
  ...
  const api = {
    id: "vibelearn",
    name: "VibeLearn (Persistent Memory)",
    version: "1.0.0",
  ...
  return {
    ...
    getCommand: (name?: string) => {
      ...
      return registeredCommands.get("vibelearn_feed");
    },
...
describe("vibeLearnPlugin", () => {
  it("registers service, commands, and event handlers on load", () => {
    ...
    assert.equal(getService().id, "vibelearn-observation-feed");
    assert.ok(getCommand("vibelearn_feed"), "feed command should be registered");
    assert.ok(getCommand("vibelearn_status"), "status command should be registered");
  ...
  describe("command handler", () => {
    it("returns not configured when no feedConfig", async () => {
      ...
      const result = await getCommand().handler({ args: "", channel: "telegram", isAuthorizedSender: true, commandBody: "/vibelearn_feed", config: {} });
    ...
    it("handles 'on' argument", async () => {
      ...
      const result = await getCommand().handler({ args: "on", channel: "telegram", isAuthorizedSender: true, commandBody: "/vibelearn_feed on", config: {} });
    ...
    it("handles 'off' argument", async () => {
      ...
      const result = await getCommand().handler({ args: "off", channel: "telegram", isAuthorizedSender: true, commandBody: "/vibelearn_feed off", config: {} });
    ...
    it("shows connection state in status output", async () => {
      ...
      const result = await getCommand().handler({ args: "", channel: "slack", isAuthorizedSender: true, commandBody: "/vibelearn_feed", config: {} });
...
describe("Observation I/O event handlers", () => {
  ...
  it("vibelearn_status command reports worker health", async () => {
    const { api, getCommand } = createMockApi({ workerPort });
    vibeLearnPlugin(api);

    const statusCmd = getCommand("vibelearn_status");
    assert.ok(statusCmd, "status command should exist");

    const result = await statusCmd.handler({ args: "", channel: "telegram", isAuthorizedSender: true, commandBody: "/vibelearn_status", config: {} });
    assert.ok(result.text.includes("Status: ok"));
    assert.ok(result.text.includes(`Port: ${workerPort}`));
  });

  it("vibelearn_status reports unreachable when worker is down", async () => {
    workerServer.close();
    await new Promise((resolve) => setTimeout(resolve, 100));
...
describe("before_prompt_build context injection", () => {
  ...
  let workerPort: number;
  let receivedRequests: Array<{ method: string; url: string; body: any }> = [];
  let contextResponse = "# VibeLearn Context\n\n## Timeline\n- Session 1: Did some work";
  ...
  beforeEach(async () => {
    receivedRequests = [];
    contextResponse = "# VibeLearn Context\n\n## Timeline\n- Session 1: Did some work";
    workerPort = await startWorkerMock();
  });
  ...
  it("returns appendSystemContext from before_prompt_build", async () => {
    ...
    assert.ok(result.appendSystemContext.includes("VibeLearn Context"), "should contain context");
  ...
  it("does not write MEMORY.md on before_agent_start", async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), "vibelearn-test-"));
    try {
      const { api, fireEvent } = createMockApi({ workerPort });
      vibeLearnPlugin(api);

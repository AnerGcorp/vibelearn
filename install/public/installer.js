...
async function runWelcome() {
  We(import_picocolors3.default.bgCyan(import_picocolors3.default.black(" vibelearn installer ")));
  ...
  if (alreadyInstalled) {
    R2.warn("Existing vibelearn installation detected.");
  }
...
async function runDependencyChecks() {
  ...
  if (!status.bunOk) {
    ...
    if (shouldInstall) {
      ...
      R2.warn("Bun is required for vibelearn. Install manually: curl -fsSL https://bun.sh/install | bash");
...
async function runProviderConfiguration() {
  const provider = await Je({
    message: "Which AI provider should vibelearn use for memory compression?",
    options: [
...
async function runSettingsConfiguration() {
  ...
  const dataDir = await Ze({
    ...
    defaultValue: "~/.vibelearn",
    placeholder: "~/.vibelearn"
  });
  if (Ct(dataDir)) {
    Ne("Installation cancelled.");
    process.exit(0);
...
function buildSettingsObject(providerConfig, settingsConfig) {
  const settings = {
    VIBELEARN_WORKER_PORT: settingsConfig.workerPort,
    VIBELEARN_WORKER_HOST: "127.0.0.1",
    VIBELEARN_DATA_DIR: expandDataDir(settingsConfig.dataDir),
    VIBELEARN_CONTEXT_OBSERVATIONS: settingsConfig.contextObservations,
    VIBELEARN_LOG_LEVEL: settingsConfig.logLevel,
    VIBELEARN_PYTHON_VERSION: settingsConfig.pythonVersion,
    VIBELEARN_PROVIDER: providerConfig.provider
  };
  if (providerConfig.provider === "claude") {
    settings.VIBELEARN_CLAUDE_AUTH_METHOD = providerConfig.claudeAuthMethod ?? "cli";
  }
  if (providerConfig.provider === "gemini") {
    if (providerConfig.apiKey) settings.VIBELEARN_GEMINI_API_KEY = providerConfig.apiKey;
    if (providerConfig.model) settings.VIBELEARN_GEMINI_MODEL = providerConfig.model;
    settings.VIBELEARN_GEMINI_RATE_LIMITING_ENABLED = providerConfig.rateLimitingEnabled !== false ? "true" : "false";
  }
  if (providerConfig.provider === "openrouter") {
    if (providerConfig.apiKey) settings.VIBELEARN_OPENROUTER_API_KEY = providerConfig.apiKey;
    if (providerConfig.model) settings.VIBELEARN_OPENROUTER_MODEL = providerConfig.model;
  }
  if (settingsConfig.chromaEnabled) {
    settings.VIBELEARN_CHROMA_MODE = settingsConfig.chromaMode ?? "local";
    if (settingsConfig.chromaMode === "remote") {
      if (settingsConfig.chromaHost) settings.VIBELEARN_CHROMA_HOST = settingsConfig.chromaHost;
      if (settingsConfig.chromaPort) settings.VIBELEARN_CHROMA_PORT = settingsConfig.chromaPort;
      if (settingsConfig.chromaSsl !== void 0) settings.VIBELEARN_CHROMA_SSL = String(settingsConfig.chromaSsl);
    }
  }
  return settings;
}
...
var MARKETPLACE_DIR = join3(homedir3(), ".claude", "plugins", "marketplaces", "anergcorp");
...
function registerMarketplace() {
  ...
  knownMarketplaces["anergcorp"] = {
    source: {
      source: "github",
      repo: "anergcorp/vibelearn"
    },
...
function registerPlugin(version) {
  ...
  const pluginCachePath = join3(PLUGINS_DIR, "cache", "anergcorp", "vibelearn", version);
  const now = (/* @__PURE__ */ new Date()).toISOString();
  installedPlugins.plugins["anergcorp@vibelearn"] = [
    {
      scope: "user",
...
function enablePluginInClaudeSettings() {
  ...
  settings.enabledPlugins["anergcorp@vibelearn"] = true;
...
async function runInstallation(selectedIDEs) {
  const tempDir = join3(tmpdir(), `vibelearn-install-${Date.now()}`);
  await Ye([
    {
      title: "Cloning vibelearn repository",
      task: async (message) => {
        message("Downloading latest release...");
        execSync3(
          `git clone --depth 1 https://github.com/anergcorp/vibelearn.git "${tempDir}"`,
          { stdio: "pipe" }
        );
  ...
  if (selectedIDEs.includes("cursor")) {
    ...
    R2.info("Run: vibelearn cursor-setup (coming soon)");
  }
}
...
var MARKETPLACE_DIR2 = join4(homedir4(), ".claude", "plugins", "marketplaces", "anergcorp");
...
async function runWorkerStartup(workerPort, dataDir) {
  ...
  const child = spawn(bunInfo.path, [workerScript], {
    ...
    env: {
      ...process.env,
      VIBELEARN_WORKER_PORT: workerPort,
      VIBELEARN_DATA_DIR: expandedDataDir
    }
  });
  child.unref();
...
function runCompletion(providerConfig, settingsConfig, selectedIDEs) {
  ...
  Le(import_picocolors8.default.green("vibelearn installed successfully!"));
}
...
async function runInstaller() {
  if (!process.stdin.isTTY) {
    ...
    console.error("Run directly: npx vibelearn-installer");
    process.exit(1);
  }

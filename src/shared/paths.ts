...
/**
 * Simple path configuration for vibelearn
...
function resolveDataDir(): string {
  ...
  const defaultDataDir = join(homedir(), '.vibelearn');
  ...
  try {
    if (existsSync(settingsPath)) {
      ...
      if (settings.VIBELEARN_DATA_DIR) {
        return settings.VIBELEARN_DATA_DIR;
      }
    }
  } catch {
...
// Note: CLAUDE_CONFIG_DIR is a Claude Code setting, not vibelearn, so leave as env var
export const CLAUDE_CONFIG_DIR = process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude');

// Plugin installation directory - respects CLAUDE_CONFIG_DIR for users with custom Claude locations
export const MARKETPLACE_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'marketplaces', 'vibelearn');
...
export const DB_PATH = join(DATA_DIR, 'vibelearn.db');
export const USER_CONFIG_PATH = join(DATA_DIR, 'config.json');

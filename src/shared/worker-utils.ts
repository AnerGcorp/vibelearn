...
// Allow env var override for users on slow systems (e.g., VIBELEARN_HEALTH_TIMEOUT_MS=10000)
const HEALTH_CHECK_TIMEOUT_MS = (() => {
  const envVal = process.env.VIBELEARN_HEALTH_TIMEOUT_MS;
  if (envVal) {
    ...
    logger.warn('SYSTEM', 'Invalid VIBELEARN_HEALTH_TIMEOUT_MS, using default', {
 ...
 * Uses VIBELEARN_WORKER_PORT from settings file or default (37777)
...
export function getWorkerPort(): number {
  ...
  const settingsPath = path.join(SettingsDefaultsManager.get('VIBELEARN_DATA_DIR'), 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  cachedPort = parseInt(settings.VIBELEARN_WORKER_PORT, 10);
  return cachedPort;
}

/**
 * Get the worker host address
 * Uses VIBELEARN_WORKER_HOST from settings file or default (127.0.0.1)
...
export function getWorkerHost(): string {
  ...
  const settingsPath = path.join(SettingsDefaultsManager.get('VIBELEARN_DATA_DIR'), 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  cachedHost = settings.VIBELEARN_WORKER_HOST;
  return cachedHost;
}
 ...
 * See: https://github.com/anergcorp/vibelearn/issues/811
 */

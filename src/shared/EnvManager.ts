/**
 * EnvManager - Centralized environment variable management for vibelearn
 *
 * Provides isolated credential storage in ~/.vibelearn/.env
 * This ensures vibelearn uses its own configured credentials,
...
// Path to vibelearn's centralized .env file
const DATA_DIR = join(homedir(), '.vibelearn');
...
// Credential keys that vibelearn manages
...
export interface VibelearnEnv {
  // Credentials (optional - empty means use CLI billing for Claude)
  ANTHROPIC_API_KEY?: string;
  GEMINI_API_KEY?: string;
  OPENROUTER_API_KEY?: string;
  VIBELEARN_API_KEY?: string;
}

/** @deprecated Use VibelearnEnv instead */
export type VibelearnEnv = VibelearnEnv;
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# vibelearn credentials',
    '# This file stores API keys for vibelearn memory agent',
    '# Edit this file or use vibelearn settings to configure',
    '',
  ];
...
/**
 * Load credentials from ~/.vibelearn/.env
 * Returns empty object if file doesn't exist (means use CLI billing)
 */
export function loadVibelearnEnv(): VibelearnEnv {
...
/**
 * Save credentials to ~/.vibelearn/.env
 */
export function saveVibelearnEnv(env: VibelearnEnv): void {
  try {
    ...
    if (env.VIBELEARN_API_KEY !== undefined) {
      if (env.VIBELEARN_API_KEY) {
        updated.VIBELEARN_API_KEY = env.VIBELEARN_API_KEY;
      } else {
        delete updated.VIBELEARN_API_KEY;
      }
    }
 ...
 *
 * If vibelearn has an explicit ANTHROPIC_API_KEY in ~/.vibelearn/.env, it's re-injected
 * after stripping, so the managed credential takes precedence over any ambient value.
 *
 * @param includeCredentials - Whether to include API keys from ~/.vibelearn/.env (default: true)
 */
export function buildIsolatedEnv(includeCredentials: boolean = true): Record<string, string> {
  ...
  if (includeCredentials) {
    ...
    // but vibelearn's .env takes precedence if configured
...
/**
 * Get a specific credential from vibelearn's .env
 * Returns undefined if not set (which means use default/CLI billing)
 */
export function getCredential(key: keyof VibelearnEnv): string | undefined {
  const env = loadVibelearnEnv();
  return env[key];
}

/**
 * Set a specific credential in vibelearn's .env
 * Pass empty string to remove the credential
 */
export function setCredential(key: keyof VibelearnEnv, value: string): void {
  const env = loadVibelearnEnv();
  env[key] = value || undefined;
  saveVibelearnEnv(env);
}

/**
 * Check if vibelearn has an Anthropic API key configured
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.vibelearn/.env)';
  }

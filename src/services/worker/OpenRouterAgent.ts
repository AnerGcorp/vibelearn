...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      if (!apiKey) {
        throw new Error('OpenRouter API key not configured. Set VIBELEARN_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
      }
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES = parseInt(settings.VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
    const MAX_ESTIMATED_TOKENS = parseInt(settings.VIBELEARN_OPENROUTER_MAX_TOKENS) || DEFAULT_MAX_ESTIMATED_TOKENS;
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/anergcorp/vibelearn',
        'X-Title': appName || 'vibelearn',
   ...
   * Issue #733: Uses centralized ~/.vibelearn/.env for credentials, not random project .env files
   */
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    const settingsPath = USER_SETTINGS_PATH;
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);

    // API key: check settings first, then centralized vibelearn .env (NOT process.env)
    // This prevents Issue #733 where random project .env files could interfere
    const apiKey = settings.VIBELEARN_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY') || '';

    // Model: from settings or default
    const model = settings.VIBELEARN_OPENROUTER_MODEL || 'xiaomi/mimo-v2-flash:free';

    // Optional analytics headers
    const siteUrl = settings.VIBELEARN_OPENROUTER_SITE_URL || '';
    const appName = settings.VIBELEARN_OPENROUTER_APP_NAME || 'vibelearn';
 ...
 * Issue #733: Uses centralized ~/.vibelearn/.env, not random project .env files
 */
export function isOpenRouterAvailable(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.VIBELEARN_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY'));
}
...
export function isOpenRouterSelected(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.VIBELEARN_PROVIDER === 'openrouter';
}

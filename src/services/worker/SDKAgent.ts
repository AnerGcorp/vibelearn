...
export class SDKAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    // Wait for agent pool slot (configurable via VIBELEARN_MAX_CONCURRENT_AGENTS)
    const settings = SettingsDefaultsManager.loadFromFile(USER_SETTINGS_PATH);
    const maxConcurrent = parseInt(settings.VIBELEARN_MAX_CONCURRENT_AGENTS, 10) || 2;
    await waitForSlot(maxConcurrent);

    // Build isolated environment from ~/.vibelearn/.env
    ...
    const queryResult = query({
      ...
      options: {
        ...
        env: isolatedEnv  // Use isolated credentials from ~/.vibelearn/.env, not process.env
      }
    });
      ...
      for await (const message of queryResult) {
        ...
        if (message.type === 'assistant') {
          ...
          if (typeof textContent === 'string' && textContent.includes('Invalid API key')) {
            throw new Error('Invalid API key: check your API key configuration in ~/.vibelearn/settings.json or ~/.vibelearn/.env');
          }
  ...
  private findClaudeExecutable(): string {
    ...
    throw new Error('Claude executable not found. Please either:\n1. Add "claude" to your system PATH, or\n2. Set CLAUDE_CODE_PATH in ~/.vibelearn/settings.json');
  }
  ...
  private getModelId(): string {
    ...
    return settings.VIBELEARN_MODEL;
  }
}

...
export class GeminiAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      if (!apiKey) {
        throw new Error('Gemini API key not configured. Set VIBELEARN_GEMINI_API_KEY in settings or GEMINI_API_KEY environment variable.');
      }
  ...
  private getGeminiConfig(): { apiKey: string; model: GeminiModel; rateLimitingEnabled: boolean } {
    ...
    const apiKey = settings.VIBELEARN_GEMINI_API_KEY || getCredential('GEMINI_API_KEY') || '';

    // Model: from settings or default, with validation
    const defaultModel: GeminiModel = 'gemini-2.5-flash';
    const configuredModel = settings.VIBELEARN_GEMINI_MODEL || defaultModel;
    ...
    const rateLimitingEnabled = settings.VIBELEARN_GEMINI_RATE_LIMITING_ENABLED !== 'false';
...
export function isGeminiAvailable(): boolean {
  const settingsPath = path.join(homedir(), '.vibelearn', 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.VIBELEARN_GEMINI_API_KEY || getCredential('GEMINI_API_KEY'));
}
...
export function isGeminiSelected(): boolean {
  const settingsPath = path.join(homedir(), '.vibelearn', 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.VIBELEARN_PROVIDER === 'gemini';
}

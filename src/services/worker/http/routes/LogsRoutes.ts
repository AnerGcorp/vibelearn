 ...
 * Logs Routes
 *
 * Handles fetching and clearing log files from ~/.vibelearn/logs/
 */
...
export class LogsRoutes extends BaseRouteHandler {
  private getLogFilePath(): string {
    const dataDir = SettingsDefaultsManager.get('VIBELEARN_DATA_DIR');
    const logsDir = join(dataDir, 'logs');
    const date = new Date().toISOString().split('T')[0];
    return join(logsDir, `vibelearn-${date}.log`);
  }

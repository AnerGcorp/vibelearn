 ...
 * Settings are stored in ~/.vibelearn/settings.json
 */
...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  /**
   * Get environment settings (from ~/.vibelearn/settings.json)
   */
  private handleGetSettings = this.wrapHandler((req: Request, res: Response): void => {
    const settingsPath = path.join(homedir(), '.vibelearn', 'settings.json');
  ...
  /**
   * Update environment settings (in ~/.vibelearn/settings.json) with validation
   */
  private handleUpdateSettings = this.wrapHandler((req: Request, res: Response): void => {
    ...
    if (existsSync(settingsPath)) {
      ...
      try {
        ...
        res.status(500).json({
          success: false,
          error: 'Settings file is corrupted. Delete ~/.vibelearn/settings.json to reset.'
        });
        return;
      }
    }
    ...
    const settingKeys = [
      ...
      'VIBELEARN_WORKER_PORT',
      'VIBELEARN_WORKER_HOST',
      // AI Provider Configuration
      'VIBELEARN_PROVIDER',
      'VIBELEARN_GEMINI_API_KEY',
      'VIBELEARN_GEMINI_MODEL',
      'VIBELEARN_GEMINI_RATE_LIMITING_ENABLED',
      // OpenRouter Configuration
      'VIBELEARN_OPENROUTER_API_KEY',
      'VIBELEARN_OPENROUTER_MODEL',
      'VIBELEARN_OPENROUTER_SITE_URL',
      'VIBELEARN_OPENROUTER_APP_NAME',
      'VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES',
      'VIBELEARN_OPENROUTER_MAX_TOKENS',
      // System Configuration
      'VIBELEARN_DATA_DIR',
      'VIBELEARN_LOG_LEVEL',
      'VIBELEARN_PYTHON_VERSION',
      'CLAUDE_CODE_PATH',
      // Token Economics
      'VIBELEARN_CONTEXT_SHOW_READ_TOKENS',
      'VIBELEARN_CONTEXT_SHOW_WORK_TOKENS',
      'VIBELEARN_CONTEXT_SHOW_SAVINGS_AMOUNT',
      'VIBELEARN_CONTEXT_SHOW_SAVINGS_PERCENT',
      // Observation Filtering
      'VIBELEARN_CONTEXT_OBSERVATION_TYPES',
      'VIBELEARN_CONTEXT_OBSERVATION_CONCEPTS',
      // Display Configuration
      'VIBELEARN_CONTEXT_FULL_COUNT',
      'VIBELEARN_CONTEXT_FULL_FIELD',
      'VIBELEARN_CONTEXT_SESSION_COUNT',
      // Feature Toggles
      'VIBELEARN_CONTEXT_SHOW_LAST_SUMMARY',
      'VIBELEARN_CONTEXT_SHOW_LAST_MESSAGE',
      'VIBELEARN_FOLDER_CLAUDEMD_ENABLED',
    ];
  ...
  private validateSettings(settings: any): { valid: boolean; error?: string } {
    // Validate VIBELEARN_PROVIDER
    if (settings.VIBELEARN_PROVIDER) {
    const validProviders = ['claude', 'gemini', 'openrouter'];
    if (!validProviders.includes(settings.VIBELEARN_PROVIDER)) {
      return { valid: false, error: 'VIBELEARN_PROVIDER must be "claude", "gemini", or "openrouter"' };
      }
    }

    // Validate VIBELEARN_GEMINI_MODEL
    if (settings.VIBELEARN_GEMINI_MODEL) {
      const validGeminiModels = ['gemini-2.5-flash-lite', 'gemini-2.5-flash', 'gemini-3-flash-preview'];
      if (!validGeminiModels.includes(settings.VIBELEARN_GEMINI_MODEL)) {
        return { valid: false, error: 'VIBELEARN_GEMINI_MODEL must be one of: gemini-2.5-flash-lite, gemini-2.5-flash, gemini-3-flash-preview' };
      }
    }

    // Validate VIBELEARN_CONTEXT_OBSERVATIONS
    if (settings.VIBELEARN_CONTEXT_OBSERVATIONS) {
      const obsCount = parseInt(settings.VIBELEARN_CONTEXT_OBSERVATIONS, 10);
      if (isNaN(obsCount) || obsCount < 1 || obsCount > 200) {
        return { valid: false, error: 'VIBELEARN_CONTEXT_OBSERVATIONS must be between 1 and 200' };
      }
    }

    // Validate VIBELEARN_WORKER_PORT
    if (settings.VIBELEARN_WORKER_PORT) {
      const port = parseInt(settings.VIBELEARN_WORKER_PORT, 10);
      if (isNaN(port) || port < 1024 || port > 65535) {
        return { valid: false, error: 'VIBELEARN_WORKER_PORT must be between 1024 and 65535' };
      }
    }

    // Validate VIBELEARN_WORKER_HOST (IP address or 0.0.0.0)
    if (settings.VIBELEARN_WORKER_HOST) {
      const host = settings.VIBELEARN_WORKER_HOST;
      ...
      if (!validHostPattern.test(host)) {
        return { valid: false, error: 'VIBELEARN_WORKER_HOST must be a valid IP address (e.g., 127.0.0.1, 0.0.0.0)' };
      }
    }

    // Validate VIBELEARN_LOG_LEVEL
    if (settings.VIBELEARN_LOG_LEVEL) {
      const validLevels = ['DEBUG', 'INFO', 'WARN', 'ERROR', 'SILENT'];
      if (!validLevels.includes(settings.VIBELEARN_LOG_LEVEL.toUpperCase())) {
        return { valid: false, error: 'VIBELEARN_LOG_LEVEL must be one of: DEBUG, INFO, WARN, ERROR, SILENT' };
      }
    }

    // Validate VIBELEARN_PYTHON_VERSION (must be valid Python version format)
    if (settings.VIBELEARN_PYTHON_VERSION) {
      const pythonVersionRegex = /^3\.\d{1,2}$/;
      if (!pythonVersionRegex.test(settings.VIBELEARN_PYTHON_VERSION)) {
        return { valid: false, error: 'VIBELEARN_PYTHON_VERSION must be in format "3.X" or "3.XX" (e.g., "3.13")' };
      }
    }
    ...
    if (settings.VIBELEARN_CONTEXT_FULL_COUNT) {
      const count = parseInt(settings.VIBELEARN_CONTEXT_FULL_COUNT, 10);
      if (isNaN(count) || count < 0 || count > 20) {
        return { valid: false, error: 'VIBELEARN_CONTEXT_FULL_COUNT must be between 0 and 20' };
      }
    }

    // Validate SESSION_COUNT (1-50)
    if (settings.VIBELEARN_CONTEXT_SESSION_COUNT) {
      const count = parseInt(settings.VIBELEARN_CONTEXT_SESSION_COUNT, 10);
      if (isNaN(count) || count < 1 || count > 50) {
        return { valid: false, error: 'VIBELEARN_CONTEXT_SESSION_COUNT must be between 1 and 50' };
      }
    }

    // Validate FULL_FIELD
    if (settings.VIBELEARN_CONTEXT_FULL_FIELD) {
      if (!['narrative', 'facts'].includes(settings.VIBELEARN_CONTEXT_FULL_FIELD)) {
        return { valid: false, error: 'VIBELEARN_CONTEXT_FULL_FIELD must be "narrative" or "facts"' };
      }
    }

    // Validate VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES
    if (settings.VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES) {
      const count = parseInt(settings.VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES, 10);
      if (isNaN(count) || count < 1 || count > 100) {
        return { valid: false, error: 'VIBELEARN_OPENROUTER_MAX_CONTEXT_MESSAGES must be between 1 and 100' };
      }
    }

    // Validate VIBELEARN_OPENROUTER_MAX_TOKENS
    if (settings.VIBELEARN_OPENROUTER_MAX_TOKENS) {
      const tokens = parseInt(settings.VIBELEARN_OPENROUTER_MAX_TOKENS, 10);
      if (isNaN(tokens) || tokens < 1000 || tokens > 1000000) {
        return { valid: false, error: 'VIBELEARN_OPENROUTER_MAX_TOKENS must be between 1000 and 1000000' };
      }
    }

    // Validate VIBELEARN_OPENROUTER_SITE_URL if provided
    if (settings.VIBELEARN_OPENROUTER_SITE_URL) {
      try {
        new URL(settings.VIBELEARN_OPENROUTER_SITE_URL);
      } catch (error) {
        // Invalid URL format
        logger.debug('SETTINGS', 'Invalid URL format', { url: settings.VIBELEARN_OPENROUTER_SITE_URL, error: error instanceof Error ? error.message : String(error) });
        return { valid: false, error: 'VIBELEARN_OPENROUTER_SITE_URL must be a valid URL' };
      }
    }

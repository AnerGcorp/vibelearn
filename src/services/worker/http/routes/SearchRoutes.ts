...
export class SearchRoutes extends BaseRouteHandler {
   ...
   */
  // Context preview — removed in VibeLearn (context injection not used)
  private handleContextPreview = this.wrapHandler((_req: Request, res: Response): void => {
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.send('');
  });

  // Context inject — removed in VibeLearn (context injection not used)
  private handleContextInject = this.wrapHandler((_req: Request, res: Response): void => {
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.send('');
  });
  ...
  private handleSearchHelp = this.wrapHandler((req: Request, res: Response): void => {
    res.json({
      ...
      examples: [
        'curl "http://localhost:37778/api/search/observations?query=authentication&limit=5"',
        'curl "http://localhost:37778/api/search/by-type?type=bugfix&limit=10"',
        'curl "http://localhost:37778/api/context/recent?project=vibelearn&limit=3"',
        'curl "http://localhost:37778/api/context/timeline?anchor=123&depth_before=5&depth_after=5"'
      ]
    });
  });
}

...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent {
    if (isOpenRouterSelected()) {
      if (isOpenRouterAvailable()) {
        ...
        throw new Error('OpenRouter provider selected but no API key configured. Set VIBELEARN_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
      }
    }
    if (isGeminiSelected()) {
      if (isGeminiAvailable()) {
        ...
        throw new Error('Gemini provider selected but no API key configured. Set VIBELEARN_GEMINI_API_KEY in settings or GEMINI_API_KEY environment variable.');
      }
    }
  ...
  private handleObservationsByClaudeId = this.wrapHandler((req: Request, res: Response): void => {
    ...
    const skipTools = new Set(settings.VIBELEARN_SKIP_TOOLS.split(',').map(t => t.trim()).filter(Boolean));

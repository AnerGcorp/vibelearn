/**
 * CursorHooksInstaller - Cursor IDE integration for vibelearn
 *
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/anergcorp/plugin/scripts/mcp-server.cjs');
    return 1;
  }
  ...
  try {
    ...
    // Add vibelearn MCP server
    config.mcpServers['vibelearn'] = {
      command: 'node',
      args: [mcpServerPath]
    };
...
export async function installCursorHooks(target: CursorInstallTarget): Promise<number> {
  console.log(`\nInstalling VibeLearn Cursor hooks (${target} level)...\n`);
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/anergcorp/plugin/scripts/worker-service.cjs');
    return 1;
  }
...
Next steps:
  1. Start vibelearn worker: vibelearn start
  2. Restart Cursor to load the hooks
  3. Check Cursor Settings → Hooks tab to verify

Context Injection:
  Context from past sessions is stored in .cursor/rules/vibelearn-context.mdc
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  if (!contextGenerated) {
    ...
    const rulesFile = path.join(rulesDir, 'vibelearn-context.mdc');
...
Use vibelearn's MCP search tools for manual memory queries.
`;
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  console.log(`\nUninstalling VibeLearn Cursor hooks (${target} level)...\n`);
  ...
  try {
    ...
    if (target === 'project') {
      const contextFile = path.join(targetDir, 'rules', 'vibelearn-context.mdc');
...
export function checkCursorHooksStatus(): number {
  ...
  for (const loc of locations) {
    ...
    if (existsSync(hooksJson)) {
      ...
      if (loc.name === 'Project') {
        const contextFile = path.join(loc.dir, 'rules', 'vibelearn-context.mdc');
        if (existsSync(contextFile)) {
  ...
  if (!anyInstalled) {
    console.log('No hooks installed. Run: vibelearn cursor install\n');
  }
...
export async function handleCursorCommand(subcommand: string, args: string[]): Promise<number> {
  switch (subcommand) {
    ...
    default: {
      console.log(`
VibeLearn Cursor Integration

Usage: vibelearn cursor <command> [options]
...
Examples:
  ...
  vibelearn cursor install user         # Install globally for user
  vibelearn cursor uninstall            # Remove from current project
  vibelearn cursor status               # Check if hooks are installed

For more info: https://docs.vibelearn.ai/cursor
      `);
      return 0;
    }
  }

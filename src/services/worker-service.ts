...
function getWorkerSpawnLockPath(): string {
  return path.join(SettingsDefaultsManager.get('VIBELEARN_DATA_DIR'), '.worker-start-attempted');
}
...
import { VibeLearnRoutes } from './worker/http/routes/VibeLearnRoutes.js';
...
export class WorkerService {
  ...
  private registerRoutes(): void {
    ...
    this.server.registerRoutes(new MemoryRoutes(this.dbManager, 'vibelearn'));
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      const modeId = 'code'; // VibeLearn uses a single fixed mode
...
async function main() {
  ...
  switch (command) {
    ...
    case 'hook': {
      ...
      if (!platform || !event) {
        console.error('Usage: vibelearn hook <platform> <event>');

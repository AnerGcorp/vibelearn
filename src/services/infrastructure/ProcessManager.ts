...
const DATA_DIR = path.join(homedir(), '.vibelearn');
const PID_FILE = path.join(DATA_DIR, 'worker.pid');

// Orphaned process cleanup patterns and thresholds
// These are vibelearn processes that can accumulate if not properly terminated
...
/**
 * Clean up orphaned vibelearn processes from previous worker sessions
 *
...
export async function cleanupOrphanedProcesses(): Promise<void> {
  ...
  try {
    if (isWindows) {
      ...
      if (!stdout.trim() || stdout.trim() === 'null') {
        logger.debug('SYSTEM', 'No orphaned vibelearn processes found (Windows)');
        return;
      }
      ...
      if (!stdout.trim()) {
        logger.debug('SYSTEM', 'No orphaned vibelearn processes found (Unix)');
        return;
      }
  ...
  logger.info('SYSTEM', 'Cleaning up orphaned vibelearn processes', {
...
/**
 * Aggressive startup cleanup for orphaned vibelearn processes.
 *
 ...
 *
 * Checks for a marker file. If absent, wipes ~/.vibelearn/chroma/ and writes
...
export function spawnDaemon(
  ...
  const env = sanitizeEnv({
    ...process.env,
    VIBELEARN_WORKER_PORT: String(port),
    ...extraEnv
  });

 ...
 * different vibelearn versions. A newer version may have added columns and
...
function repairMalformedSchema(db: Database): void {
  try {
    ...
    const scriptPath = join(tmpdir(), `vibelearn-repair-${Date.now()}.py`);
    try {
...
/**
 * VibeLearnDatabase - New entry point for the sqlite module
 *
 * Replaces SessionStore as the database coordinator.
 * Sets up bun:sqlite with optimized settings and runs all migrations.
 *
 * Usage:
 *   const db = new VibeLearnDatabase();  // uses default DB_PATH
 *   const db = new VibeLearnDatabase('/path/to/db.sqlite');
 *   const db = new VibeLearnDatabase(':memory:');  // for tests
 */
export class VibeLearnDatabase {
  public db: Database;
 ...
 * @deprecated Use VibeLearnDatabase instead for new code
 */

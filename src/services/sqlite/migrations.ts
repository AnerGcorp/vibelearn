...
/**
 * Migration 008 - VibeLearn analysis tables
 *
 * Adds all tables needed for the concept extraction, quiz generation,
 * upstream sync, and developer profile tracking pipeline.
 * All new tables use the vl_ prefix.
 * The existing session_summaries table is preserved for backward compat.
 */
export const migration008: Migration = {
  version: 8,
  up: (db: Database) => {
    // Human-readable session narratives (replaces compressed vibelearn blob format)
    db.run(`
    ...
    `);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_concepts_session ON vl_concepts(session_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_concepts_category ON vl_concepts(category)`);
    ...
    `);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_questions_session ON vl_questions(session_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_questions_concept ON vl_questions(concept_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_questions_difficulty ON vl_questions(difficulty)`);
    ...
    `);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_attempts_question ON vl_quiz_attempts(question_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_attempts_created ON vl_quiz_attempts(created_at)`);
    ...
    `);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_profile_category ON vl_developer_profile(category)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_profile_mastery ON vl_developer_profile(mastery_score)`);

    // Streak cache — server is authoritative, this is display-only
    db.run(`
    ...
    `);
    db.run(`CREATE INDEX IF NOT EXISTS idx_vl_sync_pending ON vl_sync_queue(status, attempts)`);
    ...
    console.log('✅ VibeLearn analysis tables created (migration 008)');
  },

  down: (db: Database) => {
    db.run(`DROP TABLE IF EXISTS vibelearn_session_summaries`);
    db.run(`DROP TABLE IF EXISTS vl_concepts`);
    db.run(`DROP TABLE IF EXISTS vl_questions`);
    db.run(`DROP TABLE IF EXISTS vl_quiz_attempts`);
    db.run(`DROP TABLE IF EXISTS vl_developer_profile`);
    db.run(`DROP TABLE IF EXISTS vl_daily_streaks`);
    db.run(`DROP TABLE IF EXISTS vl_sync_queue`);
    db.run(`DROP TABLE IF EXISTS vl_stack_profiles`);
  }
};

...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'vibelearn-context.mdc');
...
The following context is from vibelearn, a persistent memory system that tracks your coding sessions.

${context}

---
*Updated after last session. Use vibelearn's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'vibelearn-context.mdc');
...
/**
 * Configure vibelearn MCP server in Cursor's mcp.json
...
/**
 * Remove vibelearn MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['vibelearn']) {
      delete config.mcpServers['vibelearn'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {

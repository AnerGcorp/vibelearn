/**
 * Structured Logger for vibelearn Worker Service
...
export type Component =
  ...
  | 'CHROMA' | 'CHROMA_MCP' | 'CHROMA_SYNC' | 'FOLDER_INDEX' | 'CLAUDE_MD' | 'QUEUE'
  | 'QUIZ' | 'CONCEPTS' | 'RUBRIC' | 'ANALYZER' | 'STACK'
  | 'IMPORT' | 'CONSOLE' | 'SHUTDOWN' | 'CURSOR' | 'DEDUP' | 'SYNC' | 'TRANSCRIPT'
  | 'BRANCH' | 'PROCESS' | 'SDK_SPAWN' | 'SEARCH' | 'SECURITY' | 'VL' | 'SETTINGS'
  | 'ENV' | 'AGENTS_MD' | 'CONFIG' | 'PROJECT_NAME';
...
const DEFAULT_DATA_DIR = join(homedir(), '.vibelearn');
...
class Logger {
  ...
  private ensureLogFileInitialized(): void {
    ...
    try {
      ...
      this.logFilePath = join(logsDir, `vibelearn-${date}.log`);
    } catch (error) {
    ...
    if (this.level === null) {
      try {
        ...
        if (existsSync(settingsPath)) {
          ...
          const envLevel = (settings.VIBELEARN_LOG_LEVEL || 'INFO').toUpperCase();

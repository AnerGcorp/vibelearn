...
export async function updateFolderClaudeMdFiles(
  ...
  const limit = parseInt(settings.VIBELEARN_CONTEXT_OBSERVATIONS, 10) || 50;

  // Parse exclude paths from settings
  let folderMdExcludePaths: string[] = [];
  try {
    const parsed = JSON.parse(settings.VIBELEARN_FOLDER_MD_EXCLUDE || '[]');
    if (Array.isArray(parsed)) {
      folderMdExcludePaths = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse VIBELEARN_FOLDER_MD_EXCLUDE setting');
  }
  ...
  // See: https://github.com/anergcorp/vibelearn/issues/859

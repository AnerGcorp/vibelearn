 ...
 * - CLI: `vibelearn generate` / `vibelearn clean`
...
const DB_PATH = path.join(os.homedir(), '.vibelearn', 'vibelearn.db');
const SETTINGS_PATH = path.join(os.homedir(), '.vibelearn', 'settings.json');
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by vibelearn. Edit content outside the tags. -->');
  lines.push('');
...
function writeClaudeMdToFolder(folderPath: string, newContent: string): void {
  ...
  const startTag = '<vibelearn-context>';
  const endTag = '</vibelearn-context>';
...
export async function generateClaudeMd(dryRun: boolean): Promise<number> {
  try {
    ...
    const observationLimit = parseInt(settings.VIBELEARN_CONTEXT_OBSERVATIONS, 10) || 50;
 ...
 *
 * For each file with <vibelearn-context> tags:
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('<vibelearn-context>')) {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<vibelearn-context>[\s\S]*?<\/vibelearn-context>/g, '').trim();
